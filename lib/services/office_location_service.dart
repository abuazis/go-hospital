...
class OfficeLocationService {
  static CollectionReference _officeLocationCollection 
  = FirebaseFirestore.instance.collection('office_location');
  
  /// Melakukan simpan data collection lokasi rumah sakit
  static Stream<DocumentSnapshot> getSingle() {
    Stream<DocumentSnapshot> snapshot = _officeLocationCollection
    .doc('C7aNKLkQPElIZv7X1klb')
    .snapshots();
  ...
  static Future<List<OfficeLocation>> getResource() async {
    QuerySnapshot snapshot = await _officeLocationCollection
    .orderBy('place_name')
    .get();
