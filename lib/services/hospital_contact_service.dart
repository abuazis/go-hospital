...
class HospitalContactService {
  static CollectionReference _hospitalContactCollection 
  = FirebaseFirestore.instance.collection('hospital_contact');
    ...
    for (var document in snapshot.docs) {
      hospitalContacts.add(
        HospitalContact(
          ...
          firstContact: (document.data()['first_contact'] as List).map(
            (e) => e.toString()
          ).toList(),
          secondContact: (document.data()['second_contact'] as List).map(
            (e) => e.toString()
          ).toList(),
          thirdContact: (document.data()['third_contact'] as List).map(
            (e) => e.toString()
          ).toList(),
        ),
      );
