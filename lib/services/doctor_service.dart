    ...
    for (var document in snapshot.docs) {
      doctors.add(
        Doctor(
          ...
          education: (document.data()['education'] as List).map(
            (e) => e.toString(),
          ).toList(),
          credential: (document.data()['credential'] as List).map(
            (e) => e.toString(),
          ).toList(),
          doctorSchedule: (document.data()['schedule'] as List).map(
            (data) => DoctorSchedule.fromJson(data),
          ).toList(),
        ),
      );
    }
